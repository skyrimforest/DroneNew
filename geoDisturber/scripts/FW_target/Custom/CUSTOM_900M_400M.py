...
import CUSTOM_900M_400M_change_sink as change_sink  # embedded python module
import CUSTOM_900M_400M_receive_para as receive_para  # embedded python module
import time
import threading
...
class CUSTOM_900M_400M(gr.top_block):

    def __init__(self, END_1=900000000, END_2=430500000, START_1=850000000, START_2=430000000, uri='192.168.1.7'):
        gr.top_block.__init__(self, "CUSTOM_900M_400M", catch_exceptions=True)
        ...
        if true_en:
            self.iio_fmcomms2_sink_0.set_attenuation(0, 21.0)
        if not true_en:
            self.iio_fmcomms2_sink_0.set_attenuation(1, 20.5)
...
def argument_parser():
    ...
    parser.add_argument(
        "--END-1", dest="END_1", type=intx, default=900000000,
        help="Set END_1 [default=%(default)r]")
    parser.add_argument(
        "--END-2", dest="END_2", type=intx, default=430500000,
        help="Set END_2 [default=%(default)r]")
    parser.add_argument(
        "--START-1", dest="START_1", type=intx, default=850000000,
        help="Set START_1 [default=%(default)r]")
    parser.add_argument(
        "--START-2", dest="START_2", type=intx, default=430000000,
...
def main(top_block_cls=CUSTOM_900M_400M, options=None):
    if options is None:
