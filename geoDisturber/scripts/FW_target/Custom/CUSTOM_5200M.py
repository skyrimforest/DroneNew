...
import CUSTOM_5200M_change_sink as change_sink  # embedded python module
import CUSTOM_5200M_receive_para as receive_para  # embedded python module
import time
import threading
...
class CUSTOM_5200M(gr.top_block):

    def __init__(self, END_1=5350000000, START_1=5150000000, uri='192.168.1.12'):
        gr.top_block.__init__(self, "CUSTOM_5200M", catch_exceptions=True)
        ...
        # Variables
        ##################################################
        self.default = default = 5792500000
        self.hf_function_probe = hf_function_probe = 0
        self.freq_b = freq_b = receive_para.work(START_1,END_1)[1]
        self.freq_a = freq_a = default
        self.bbb = bbb = 1
        self.aaa = aaa = 0
        self.true_en = true_en = change_sink.work(hf_function_probe,aaa, bbb, freq_a, freq_b, freq_b[0],0)[1]
        self.SW_feq = SW_feq = receive_para.work(START_1,END_1)[0]
        self.SAMP = SAMP = receive_para.work(START_1,END_1)[0]
        self.LO_feq = LO_feq = change_sink.work(hf_function_probe,aaa, bbb, freq_a, freq_b, freq_b[0],0)[0]
        ...
        if true_en:
            self.iio_fmcomms2_sink_0.set_attenuation(0, 18.0)
        if not true_en:
    ...
    def set_END_1(self, END_1):
        ...
        self.set_freq_b(receive_para.work(self.START_1,self.END_1)[1])
    ...
    def set_hf_function_probe(self, hf_function_probe):
        ...
        self.set_LO_feq(change_sink.work(self.hf_function_probe,self.aaa, self.bbb, self.freq_a, self.freq_b, self.freq_b[0],0)[0])
        self.set_true_en(change_sink.work(self.hf_function_probe,self.aaa, self.bbb, self.freq_a, self.freq_b, self.freq_b[0],0)[1])
...
def argument_parser():
    ...
    parser.add_argument(
        "--END-1", dest="END_1", type=intx, default=5350000000,
        help="Set END_1 [default=%(default)r]")
    parser.add_argument(
        "--START-1", dest="START_1", type=intx, default=5150000000,
...
def main(top_block_cls=CUSTOM_5200M, options=None):
    if options is None:
