...
import CUSTOM_1500M_1400M_change_sink as change_sink  # embedded python module
import CUSTOM_1500M_1400M_receive_para as receive_para  # embedded python module
import time
import threading
...
class CUSTOM_1500M_1400M(gr.top_block):

    def __init__(self, END_1=1630000000, END_2=1455000000, START_1=1480000000, START_2=1415000000, uri='192.168.1.11'):
        gr.top_block.__init__(self, "CUSTOM_1500M_1400M", catch_exceptions=True)
        ...
        self.freq_b = freq_b = receive_para.work(START_1,END_1,START_2,END_2)[2]
        self.freq_a = freq_a = receive_para.work(START_1,END_1,START_2,END_2)[1]
        self.bbb = bbb = 1
        self.aaa = aaa = 1
        self.true_en = true_en = change_sink.work(hf_function_probe,aaa, bbb, freq_a, freq_b, freq_a[0],1)[1]
        self.SW_feq = SW_feq = receive_para.work(START_1,END_1,START_2,END_2)[0]
        self.SAMP = SAMP = receive_para.work(START_1,END_1,START_2,END_2)[0]
        self.LO_feq = LO_feq = change_sink.work(hf_function_probe,aaa, bbb, freq_a, freq_b, freq_a[0],1)[0]
        self.BW = BW = receive_para.work(START_1,END_1,START_2,END_2)[0]
        ...
        if true_en:
            self.iio_fmcomms2_sink_0.set_attenuation(0, 12.6)
        if not true_en:
            self.iio_fmcomms2_sink_0.set_attenuation(1, 10.6)
    ...
    def set_END_1(self, END_1):
        self.END_1 = END_1
        self.set_BW(receive_para.work(self.START_1,self.END_1,self.START_2,self.END_2)[0])
        self.set_SAMP(receive_para.work(self.START_1,self.END_1,self.START_2,self.END_2)[0])
        self.set_SW_feq(receive_para.work(self.START_1,self.END_1,self.START_2,self.END_2)[0])
        self.set_freq_a(receive_para.work(self.START_1,self.END_1,self.START_2,self.END_2)[1])
        self.set_freq_b(receive_para.work(self.START_1,self.END_1,self.START_2,self.END_2)[2])

    def get_END_2(self):
        return self.END_2

    def set_END_2(self, END_2):
        self.END_2 = END_2
        self.set_BW(receive_para.work(self.START_1,self.END_1,self.START_2,self.END_2)[0])
        self.set_SAMP(receive_para.work(self.START_1,self.END_1,self.START_2,self.END_2)[0])
        self.set_SW_feq(receive_para.work(self.START_1,self.END_1,self.START_2,self.END_2)[0])
    ...
    def get_START_2(self):
        return self.START_2

    def set_START_2(self, START_2):
        self.START_2 = START_2
        self.set_BW(receive_para.work(self.START_1,self.END_1,self.START_2,self.END_2)[0])
        self.set_SAMP(receive_para.work(self.START_1,self.END_1,self.START_2,self.END_2)[0])
        self.set_SW_feq(receive_para.work(self.START_1,self.END_1,self.START_2,self.END_2)[0])
...
def argument_parser():
    ...
    parser.add_argument(
        "--END-1", dest="END_1", type=intx, default=1630000000,
        help="Set END_1 [default=%(default)r]")
    parser.add_argument(
        "--END-2", dest="END_2", type=intx, default=1455000000,
        help="Set END_2 [default=%(default)r]")
    parser.add_argument(
        "--START-1", dest="START_1", type=intx, default=1480000000,
        help="Set START_1 [default=%(default)r]")
    parser.add_argument(
        "--START-2", dest="START_2", type=intx, default=1415000000,
        help="Set START_2 [default=%(default)r]")
    parser.add_argument(
...
def main(top_block_cls=CUSTOM_1500M_1400M, options=None):
    if options is None:
        options = argument_parser().parse_args()
    tb = top_block_cls(END_1=options.END_1, END_2=options.END_2, START_1=options.START_1, START_2=options.START_2, uri=options.uri)
