...
import SIN_2400M_change_sink as change_sink  # embedded python module
import time
import threading
...
class SIN_2400M(gr.top_block):

    def __init__(self, uri='192.168.1.13'):
        gr.top_block.__init__(self, "SIN_2400M", catch_exceptions=True)
        ...
        self.freq_b = freq_b = 0
        self.freq_a = freq_a = 2412500000,2437500000,2462500000,2487500000
        self.bbb = bbb = 0
        self.atten_b = atten_b = 0
        self.atten_a = atten_a = 18.6,18.9,19,18.5
        self.aaa = aaa = 1
        self.true_en = true_en = change_sink.work(hf_function_probe,aaa, bbb, freq_a, freq_b,  atten_a, atten_b,2412500000,1,18.6)[1]
        self.loss = loss = 3
        self.atten = atten = change_sink.work(hf_function_probe,aaa, bbb, freq_a, freq_b,  atten_a, atten_b,2412500000,1,18.6)[2]
        self.SW_feq = SW_feq = 25000000
        self.SAMP = SAMP = 25000000
        self.LO_feq = LO_feq = change_sink.work(hf_function_probe,aaa, bbb, freq_a, freq_b,  atten_a, atten_b,2412500000,1,18.6)[0]
        self.BW = BW = 30000000
        ...
        if true_en:
            self.iio_fmcomms2_sink_0.set_attenuation(0, atten-4+loss)
        if not true_en:
    ...
    def set_hf_function_probe(self, hf_function_probe):
        ...
        self.set_LO_feq(change_sink.work(self.hf_function_probe,self.aaa, self.bbb, self.freq_a, self.freq_b,  self.atten_a, self.atten_b,2412500000,1,18.6)[0])
        self.set_atten(change_sink.work(self.hf_function_probe,self.aaa, self.bbb, self.freq_a, self.freq_b,  self.atten_a, self.atten_b,2412500000,1,18.6)[2])
        self.set_true_en(change_sink.work(self.hf_function_probe,self.aaa, self.bbb, self.freq_a, self.freq_b,  self.atten_a, self.atten_b,2412500000,1,18.6)[1])
    ...
    def set_loss(self, loss):
        self.loss = loss
        self.iio_fmcomms2_sink_0.set_attenuation(0, self.atten-4+self.loss)
...
def main(top_block_cls=SIN_2400M, options=None):
    if options is None:
