...
import DOU_900M_400M_change_sink as change_sink  # embedded python module
import time
import threading
...
class DOU_900M_400M(gr.top_block):

    def __init__(self, uri='192.168.1.7'):
        gr.top_block.__init__(self, "DOU_900M_400M", catch_exceptions=True)
        ...
        self.atten_a = atten_a = 21.3,20.9,20.5,21,21,21
        self.aaa = aaa = 1
        self.true_en = true_en = change_sink.work(hf_function_probe,aaa, bbb, freq_a, freq_b, atten_a, atten_b, 845000000, 1, 21.3)[1]
        self.atten = atten = change_sink.work(hf_function_probe,aaa, bbb, freq_a, freq_b, atten_a, atten_b, 845000000, 1, 21.3)[2]
        self.SW_feq = SW_feq = 20000000
        self.SAMP = SAMP = 20000000
        self.LO_feq = LO_feq = change_sink.work(hf_function_probe,aaa, bbb, freq_a, freq_b, atten_a, atten_b, 845000000, 1, 21.3)[0]
        self.BW = BW = 30000000
    ...
    def set_hf_function_probe(self, hf_function_probe):
        ...
        self.set_LO_feq(change_sink.work(self.hf_function_probe,self.aaa, self.bbb, self.freq_a, self.freq_b, self.atten_a, self.atten_b, 845000000, 1, 21.3)[0])
        self.set_atten(change_sink.work(self.hf_function_probe,self.aaa, self.bbb, self.freq_a, self.freq_b, self.atten_a, self.atten_b, 845000000, 1, 21.3)[2])
        self.set_true_en(change_sink.work(self.hf_function_probe,self.aaa, self.bbb, self.freq_a, self.freq_b, self.atten_a, self.atten_b, 845000000, 1, 21.3)[1])
...
def main(top_block_cls=DOU_900M_400M, options=None):
    if options is None:
