...
import SIN_1400M_change_sink as change_sink  # embedded python module
import time
import threading
...
class SIN_1400M(gr.top_block):

    def __init__(self, uri='192.168.1.11'):
        gr.top_block.__init__(self, "SIN_1400M", catch_exceptions=True)
        ...
        self.freq_b = freq_b = 1435000000,1435000000
        self.freq_a = freq_a = 1495000000,1525000000,1555000000,1585000000,1615000000
        self.bbb = bbb = 1
        self.atten_b = atten_b = 17.6,17.6
        self.atten_a = atten_a = 12.4,13.2,12.6,10.6,8.8
        self.aaa = aaa = 0
        self.true_en = true_en = change_sink.work(hf_function_probe,aaa, bbb, freq_a, freq_b, atten_a, atten_b,1425000000,0,17.6)[1]
        self.atten = atten = change_sink.work(hf_function_probe,aaa, bbb, freq_a, freq_b, atten_a, atten_b,1425000000,0,17.6)[2]
        self.SW_feq = SW_feq = 30000000
        self.SAMP = SAMP = 30000000
        self.LO_feq = LO_feq = change_sink.work(hf_function_probe,aaa, bbb, freq_a, freq_b, atten_a, atten_b,1425000000,0,17.6)[0]
        self.BW = BW = 30000000
        ...
        if not true_en:
            self.iio_fmcomms2_sink_0.set_attenuation(1, atten-7)
    ...
    def set_hf_function_probe(self, hf_function_probe):
        ...
        self.set_LO_feq(change_sink.work(self.hf_function_probe,self.aaa, self.bbb, self.freq_a, self.freq_b, self.atten_a, self.atten_b,1425000000,0,17.6)[0])
        self.set_atten(change_sink.work(self.hf_function_probe,self.aaa, self.bbb, self.freq_a, self.freq_b, self.atten_a, self.atten_b,1425000000,0,17.6)[2])
        self.set_true_en(change_sink.work(self.hf_function_probe,self.aaa, self.bbb, self.freq_a, self.freq_b, self.atten_a, self.atten_b,1425000000,0,17.6)[1])
    ...
    def set_atten(self, atten):
        self.atten = atten
        self.iio_fmcomms2_sink_0.set_attenuation(1, self.atten-7)
...
def main(top_block_cls=SIN_1400M, options=None):
    if options is None:
