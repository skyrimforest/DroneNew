...
import SIN_1500M_change_sink as change_sink  # embedded python module
import time
import threading
...
class SIN_1500M(gr.top_block):

    def __init__(self, uri='192.168.1.11'):
        gr.top_block.__init__(self, "SIN_1500M", catch_exceptions=True)
        ...
        self.bbb = bbb = 0
        self.atten_b = atten_b = 8.1,8.8
        self.atten_a = atten_a = 19.8,19.4,19.1,19.2,19.2
        self.aaa = aaa = 1
        ...
        if true_en:
            self.iio_fmcomms2_sink_0.set_attenuation(0, atten-7)
        if not true_en:
    ...
    def set_atten(self, atten):
        self.atten = atten
        self.iio_fmcomms2_sink_0.set_attenuation(0, self.atten-7)
...
def main(top_block_cls=SIN_1500M, options=None):
    if options is None:
