...
import SIN_900M_change_sink as change_sink  # embedded python module
import time
import threading
...
class SIN_900M(gr.top_block):

    def __init__(self, uri='192.168.1.7'):
        gr.top_block.__init__(self, "SIN_900M", catch_exceptions=True)
        ...
        self.freq_a = freq_a = 850000000,880000000,910000000,940000000
        self.bbb = bbb = 0
        self.atten_b = atten_b = 0.5,0
        self.atten_a = atten_a = 21.3,20.5,21,21
        self.aaa = aaa = 1
        self.true_en = true_en = change_sink.work(hf_function_probe,aaa, bbb, freq_a, freq_b, atten_a, atten_b, 850000000,1,21.3)[1]
        self.atten = atten = change_sink.work(hf_function_probe,aaa, bbb, freq_a, freq_b, atten_a, atten_b, 850000000,1,21.3)[2]
        self.SW_feq = SW_feq = 30000000
        self.SAMP = SAMP = 30000000
        self.LO_feq = LO_feq = change_sink.work(hf_function_probe,aaa, bbb, freq_a, freq_b, atten_a, atten_b, 850000000,1,21.3)[0]
        self.BW = BW = 30000000
    ...
    def set_hf_function_probe(self, hf_function_probe):
        ...
        self.set_LO_feq(change_sink.work(self.hf_function_probe,self.aaa, self.bbb, self.freq_a, self.freq_b, self.atten_a, self.atten_b, 850000000,1,21.3)[0])
        self.set_atten(change_sink.work(self.hf_function_probe,self.aaa, self.bbb, self.freq_a, self.freq_b, self.atten_a, self.atten_b, 850000000,1,21.3)[2])
        self.set_true_en(change_sink.work(self.hf_function_probe,self.aaa, self.bbb, self.freq_a, self.freq_b, self.atten_a, self.atten_b, 850000000,1,21.3)[1])
...
def main(top_block_cls=SIN_900M, options=None):
    if options is None:
