...
import DOU_5800M_5200M_change_sink as change_sink  # embedded python module
import time
import threading
...
class DOU_5800M_5200M(gr.top_block):

    def __init__(self, uri='192.168.1.12'):
        gr.top_block.__init__(self, "DOU_5800M_5200M", catch_exceptions=True)
        ...
        self.bbb = bbb = 1
        self.atten_b = atten_b = 12.5,12.5,12.7,12.3,12.7,13.2,12.6,12.6
        self.atten_a = atten_a = 4.2,4.2,3.5,3,3,3
        self.aaa = aaa = 1
        self.true_en = true_en = change_sink.work(hf_function_probe,aaa, bbb, freq_a, freq_b, atten_a, atten_b, 5727000000,1,4.2)[1]
        self.atten = atten = change_sink.work(hf_function_probe,aaa, bbb, freq_a, freq_b, atten_a, atten_b, 5727000000,1,4.2)[2]
        self.SW_feq = SW_feq = 26000000
        self.SAMP = SAMP = 26000000
        self.LO_feq = LO_feq = change_sink.work(hf_function_probe,aaa, bbb, freq_a, freq_b, atten_a, atten_b, 5727000000,1,4.2)[0]
        self.BW = BW = 30000000
        ...
        if true_en:
            self.iio_fmcomms2_sink_0.set_attenuation(0, atten)
        if not true_en:
            self.iio_fmcomms2_sink_0.set_attenuation(1, atten)
    ...
    def set_hf_function_probe(self, hf_function_probe):
        ...
        self.set_LO_feq(change_sink.work(self.hf_function_probe,self.aaa, self.bbb, self.freq_a, self.freq_b, self.atten_a, self.atten_b, 5727000000,1,4.2)[0])
        self.set_atten(change_sink.work(self.hf_function_probe,self.aaa, self.bbb, self.freq_a, self.freq_b, self.atten_a, self.atten_b, 5727000000,1,4.2)[2])
        self.set_true_en(change_sink.work(self.hf_function_probe,self.aaa, self.bbb, self.freq_a, self.freq_b, self.atten_a, self.atten_b, 5727000000,1,4.2)[1])
    ...
    def set_atten(self, atten):
        self.atten = atten
        self.iio_fmcomms2_sink_0.set_attenuation(0, self.atten)
        self.iio_fmcomms2_sink_0.set_attenuation(1, self.atten)
...
def main(top_block_cls=DOU_5800M_5200M, options=None):
    if options is None:
