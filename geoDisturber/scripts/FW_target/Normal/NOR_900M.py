...
import NOR_900M_change_sink as change_sink  # embedded python module
import NOR_900M_receive_para as receive_para  # embedded python module
import time
import threading
...
class NOR_900M(gr.top_block):

    def __init__(self, uri='192.168.1.7'):
        gr.top_block.__init__(self, "NOR_900M", catch_exceptions=True)
        ...
        self.START_1 = START_1 = 840500000
        self.END_1 = END_1 = 845000000
        ...
        if true_en:
            self.iio_fmcomms2_sink_0.set_attenuation(0, 20.5)
        if not true_en:
...
def main(top_block_cls=NOR_900M, options=None):
    if options is None:
