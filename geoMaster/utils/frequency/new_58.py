...
gr_proc = subprocess.Popen([sys.executable, "/home/uhd/gps-sdr-sim/123/D58.py"])

# 等 IPC 文件出现（最多等 2 秒）
for _ in range(40):
    if os.path.exists("/tmp/fft1.ipc"):
        break
    time.sleep(0.05)

IPC_ADDR = "ipc:///tmp/fft1.ipc"  # 与 GRC 中 PUB Sink 完全一致
VEC_LEN = 4096  # 一帧的点数
...
# matplotlib 画布
fig = plt.figure("Live Spectrogram")
ax = plt.gca()
